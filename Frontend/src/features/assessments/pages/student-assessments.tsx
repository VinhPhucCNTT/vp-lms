...
export function StudentAssessments() {
  ...
  const stats = {
    ...
    averageScore:
      ...
      Math.max(userAssessments.filter((a) => a.score).length, 1),
    passing: userAssessments.filter((a) => a.score && a.score >= a.passingScore)
      .length,
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Take quizzes and track your performance"
        breadcrumbs={[
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <p className="text-2xl font-bold">
              {userAssessments.filter((a) => a.status === "not-started").length}
            </p>
          </CardContent>
        </Card>
        <Card>
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-2xl font-bold">
              {userAssessments.filter((a) => a.status === "overdue").length}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-4">
        {userAssessments.map((assessment) => (
          <Card
            key={assessment.id}
            className={cn(
              assessment.status === "overdue" && "border-destructive/50"
            )}
          >
              ...
              <div className="flex items-start justify-between">
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge className={cn(statusColors[assessment.status])}>
                      {assessment.status}
                    </Badge>
                  </div>
      ...
      <Dialog
        open={!!selectedAssessment && !showResults}
      ...
      >
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Answer all questions carefully. Once submitted, you cannot change
              your answers.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-6">
                  ...
                  {sampleQuestions[currentQuestion].options.map(
                    (option, index) => (
                      <button
                        ...
                        onClick={() =>
                          handleAnswer(
                            sampleQuestions[currentQuestion].id,
                            index
                          )
                        }
                        className={cn(
      ...
      >
        <DialogContent className="max-w-md text-center">
          <DialogHeader>
            <DialogTitle
              className={cn(score >= 70 ? "text-success" : "text-destructive")}
            >
              {score >= 70 ? "Congratulations!" : "Try Again"}
            </DialogTitle>
          </DialogHeader>
          <div className="py-6">
            <p className="mb-2 text-5xl font-bold">{score}%</p>
            <p className="text-muted-foreground">
              You answered {correctCount} out of {sampleQuestions.length}{" "}
              questions correctly
            </p>
          </div>
          <DialogFooter>
