...
export function StudentAssignments() {
  ...
  const allAssignments: AssignmentWithStatus[] = assignments
    .filter((a) =>
      enrolledCourseIds.some((id) => courses.find((c) => c.id === id)?.id)
    )
    .map((a) => {
  ...
  const allAssessments: AssessmentWithStatus[] = assessments
    ...
    .map((a) => {
      const course = courses.find((c) => c.id === "cs-101")!
      const statuses: AssessmentWithStatus["status"][] = [
        "not-started",
        "in-progress",
        "completed",
        "overdue",
  ...
  const avgScore =
    ...
    Math.max(allAssignments.filter((a) => a.score).length, 1)
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <Progress
              value={(completedCount / allAssignments.length) * 100}
              className="mt-2"
            />
            <p className="mt-1 text-xs text-muted-foreground">
              {completedCount}/{allAssignments.length} completed
            </p>
          </CardContent>
        </Card>
      </div>
                ...
                <CardContent className="p-4">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <Link
                        to={`/student/courses/${assignment.course.code.toLowerCase().replace(" ", "-")}`}
                      >
                        <Button
                          size="sm"
                          variant={
                ...
                <div className="flex items-start justify-between">
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge
                        className={cn(
                          assessmentStatusColors[assessment.status]
                        )}
                      >
                        {assessment.status}
                      </Badge>
