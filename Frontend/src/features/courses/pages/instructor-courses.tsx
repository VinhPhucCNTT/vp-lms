...
export function InstructorCourses() {
  const { data: courses, loading, error, reload } = useApi<Course[]>(() => courseApi.getInstructorCourses());
        ...
        <CardFooter className="pt-2 border-t">
          <div className="flex items-center gap-2 ml-auto">
            ...
            <Button size="sm" variant="secondary" asChild>
              <Link to={`/instructor/courses/${course.id}`}>View Course</Link>
            </Button>
          </div>
        </CardFooter>
      </Card>
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="all">
        ...
        <TabsContent value="published" className="mt-6">
          {published.length === 0 ? (
            <EmptyState message="No published courses." />
          ) : (
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {published.map((c) => <CourseRow key={c.id} course={c} />)}
            </div>
          )}
        </TabsContent>
        ...
        <TabsContent value="draft" className="mt-6">
          {drafts.length === 0 ? (
            <EmptyState message="No unpublished courses." />
          ) : (
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {drafts.map((c) => <CourseRow key={c.id} course={c} />)}
            </div>
          )}
        </TabsContent>
      </Tabs>
