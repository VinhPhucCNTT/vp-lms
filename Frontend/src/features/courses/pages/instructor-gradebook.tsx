...
export function InstructorGradebook() {
  ...
  const studentGrades: StudentGrade[] = students
    .slice(0, 6)
    .map((student, index) => ({
      ...
      assignments: [
        ...
        {
          title: "A2: Sorting Implementation",
          score: Math.floor(Math.random() * 25) + 75,
          maxScore: 100,
        },
        {
          title: "A3: Binary Trees",
  ...
  const passingRate = Math.round(
    (studentGrades.filter((g) => g.percentage >= 60).length /
      studentGrades.length) *
      100
  )
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium">A/B Students</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-bold">
              {studentGrades.filter((g) => g.percentage >= 80).length}
            </p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <p className="text-2xl font-bold text-warning">
              {studentGrades.filter((g) => g.percentage < 70).length}
            </p>
          </CardContent>
        </Card>
      </div>
                ...
                {filteredGrades.map((student) => (
                  <TableRow key={student.studentId}>
                    ...
                    <TableCell>
                      <Badge
                        className={cn(
                          gradeColors[student.finalGrade[0]] + " font-bold"
                        )}
                      >
                        {student.finalGrade}
                      </Badge>
